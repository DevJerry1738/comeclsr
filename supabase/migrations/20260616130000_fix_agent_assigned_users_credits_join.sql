-- 20260615c_fix_agent_assigned_users_credits_join.sql
-- Fix get_assigned_users to correctly retrieve the user's credits balance by using user_id instead of auth.uid() in the JOIN.
...
BEGIN
  ...
  WITH all_users AS (
    ...
    LEFT JOIN public.user_credits uc ON up.id = uc.user_id  -- Note: Joined correctly on up.id = uc.user_id
    ...
    JOIN public.user_credits uc ON up.id = uc.user_id  -- Note: Joined correctly on up.id = uc.user_id
