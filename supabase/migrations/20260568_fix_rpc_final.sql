-- Migration: Fix RPC function ambiguous column references (Final Fix)
-- Issue: Both agent_get_conversation_with_user and conversation_get_messages have ambiguous "id" references
-- Solution: Use explicit table aliases in the RETURN QUERY SELECT without changing column names
...
BEGIN
  ...
  IF NOT v_is_admin AND NOT EXISTS (
    SELECT 1 FROM public.conversations WHERE id = v_conversation_id AND user_id = v_user_id
  ) THEN
